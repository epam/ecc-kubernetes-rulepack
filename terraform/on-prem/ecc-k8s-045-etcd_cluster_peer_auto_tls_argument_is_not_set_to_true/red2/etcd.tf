resource "null_resource" "deploy_cluster" {
  ...
  provisioner "local-exec" {
    when        = destroy
    command     = "kind delete cluster -n cluster-045-red-2"
    interpreter = ["/bin/bash", "-c"]
  }
}
...
resource "null_resource" "this" {
  ...
  provisioner "local-exec" {
    command     = <<CMD
        kubectl config set-context  kind-cluster-045-red-2
        kind_control_plane_1_id=`docker ps | grep cluster-045-red-2-control-plane$ | awk '{print $1}'`
        echo "$kind_control_plane_1_id"
        kind_control_plane_2_id=`docker ps | grep cluster-045-red-2-control-plane2$ | awk '{print $1}'`
