...
resource "null_resource" "this" {
  provisioner "local-exec" {
    command     = <<CMD
    POD_NAME=$(kubectl get pods -l ComplianceStatus=Red3  -o jsonpath='{.items[0].metadata.name}')
      ...
      -d '
    {
        "spec":
        {
            ...
            [
                {
                    "name": "ephemeral-container-061-red3",
                    "command": ["powershell.exe"],
                    "image": "mcr.microsoft.com/powershell:lts-nanoserver-1809",
                    "targetContainerName": "container-windows-server-iis-061-red3",
                    "stdin": true,
                    "tty": true,
                    "securityContext": {
                        "windowsOptions": {
